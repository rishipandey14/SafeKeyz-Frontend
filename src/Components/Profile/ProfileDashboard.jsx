...
const ProfileDashboard = () => {
  ...
  const ProgressBar = ({ percentage }) => (
    <div className="w-full bg-gray-200 rounded-full h-2 overflow-hidden shadow-inner">
      <div className="bg-green-500 h-2 rounded-full transition-all duration-500 ease-in-out" style={{ width: `${Math.min(percentage, 100)}%` }} />
    </div>
  );
    ...
    <RequireAuth>
      <div className="min-h-screen text-gray-900 p-8 font-sans select-none bg-gray-50">
        <div className="max-w-4xl mx-auto mb-12 bg-white rounded-2xl shadow-md flex items-center gap-6 p-6 md:p-10 border border-gray-200 hover:shadow-lg transition-shadow duration-300">
          <img src={userData.photoUrl} alt={userData.firstName} className="w-24 h-24 md:w-28 md:h-28 rounded-full border-4 border-green-500 object-cover shadow-md" />
          <div>
            <h1 className="text-3xl md:text-4xl font-extrabold tracking-wide">
              Welcome back,{" "}
              <span className="text-green-600">{userData.firstName}</span>
            </h1>
            <p className="text-gray-600 mt-1 text-sm md:text-base">Here's a quick overview of your vault and activity.</p>
          </div>
        </div>
        ...
        <div className="max-w-6xl mx-auto mt-12 bg-white rounded-2xl p-6 md:p-8 shadow-md border border-gray-200">
          <h2 className="text-2xl md:text-3xl font-semibold mb-5 text-green-600 border-b border-gray-200 pb-2">Recent Activities</h2>
          <ul className="list-disc list-inside space-y-2 text-gray-700 max-h-60 overflow-y-auto">
            {recentActivities.map((activity, idx) => (
              <li key={idx} className="hover:text-green-600 transition-colors cursor-pointer select-text" title={activity}>
                {activity}
              </li>
            ))}
          </ul>
