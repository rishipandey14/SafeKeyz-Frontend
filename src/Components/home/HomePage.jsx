...
const HomePage = () => {
  ...
  const features = [
    { icon: Lock, title: "Military-Grade Encryption", description: "Your passwords are encrypted with AES-256, ensuring maximum security" },
    { icon: Share2, title: "Secure Sharing", description: "Share passwords safely with team members with granular permissions" },
    { icon: Zap, title: "Lightning Fast", description: "Instant access to your vault from any device, anytime" },
    { icon: Eye, title: "Privacy First", description: "We never store, see, or access your passwords. Only you do" },
    { icon: Users, title: "Team Collaboration", description: "Manage passwords across your entire organization securely" },
    { icon: Shield, title: "Always Protected", description: "Advanced security features keep your data safe 24/7" },
  ];

  const testimonials = [
    { name: "Sarah Johnson", role: "Security Officer", text: "SafeKeyz has transformed how we manage passwords. It's secure, intuitive, and reliable.", avatar: "https://api.dicebear.com/7.x/avataaars/svg?seed=Sarah" },
    { name: "Mike Chen", role: "Startup Founder", text: "The best password manager we've used. Great features and excellent support.", avatar: "https://api.dicebear.com/7.x/avataaars/svg?seed=Mike" },
    { name: "Emma Davis", role: "HR Manager", text: "Perfect for team collaboration. Made password management effortless.", avatar: "https://api.dicebear.com/7.x/avataaars/svg?seed=Emma" },
  ];
  ...
  const pricingPlans = [
    {
      ...
      features: ["Up to 50 passwords", "Basic encryption", "1 device", "Email support"],
    ...
    <div className="bg-white">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-12 items-center">
            <div className="space-y-8">
              ...
              <div className="flex flex-col sm:flex-row gap-4 pt-4">
                {user ? (
                  <Link to="/vault" className="inline-flex items-center justify-center gap-2 bg-green-600 hover:bg-green-700 text-white px-8 py-4 rounded-lg font-semibold transition-all duration-200 shadow-lg hover:shadow-xl">
                    Open Your Vault
                    <ArrowRight size={20} />
                  </Link>
                ) : (
                  <>
                    <Link to="/login" state={{ isLoginForm: false }} className="inline-flex items-center justify-center gap-2 bg-green-600 hover:bg-green-700 text-white px-8 py-4 rounded-lg font-semibold transition-all duration-200 shadow-lg hover:shadow-xl">
                      Get Started Free
                      <ArrowRight size={20} />
                    </Link>
                    <a href="#features" className="inline-flex items-center justify-center gap-2 border-2 border-gray-300 hover:border-green-600 text-gray-900 hover:text-green-600 px-8 py-4 rounded-lg font-semibold transition-all duration-200">
                      Learn More
                    </a>
                  </>
                )}
      ...
      <section id="features" className="py-20 bg-gray-50 border-t border-gray-200">
        <div className="max-w-7xl mx-auto px-6">
          <div className="text-center mb-16">
            ...
            <p className="text-xl text-gray-600 max-w-2xl mx-auto">Everything you need to keep your passwords secure and organized</p>
          </div>
      ...
      <section className="py-20 bg-white">
        <div className="max-w-7xl mx-auto px-6">
          <div className="text-center mb-16">
            ...
            <p className="text-xl text-gray-600 max-w-2xl mx-auto">Simple, secure, and straightforward</p>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
            {[
              ...
              <div key={idx} className="text-center">
                <div className="w-12 h-12 bg-green-600 text-white rounded-full flex items-center justify-center font-bold text-lg mx-auto mb-4">{item.step}</div>
          ...
          <div className="relative max-w-xl mx-auto">
            ...
            <div className="flex justify-center gap-3 mt-8">
              {testimonials.map((_, idx) => (
                <button key={idx} onClick={() => setCurrentTestimonial(idx)} className={`transition-all duration-300 rounded-full ${currentTestimonial === idx ? 'w-8 h-3 bg-green-600' : 'w-3 h-3 bg-gray-300 hover:bg-gray-400'}`} aria-label={`Go to testimonial ${idx + 1}`} />
              ))}
            </div>

            <button onClick={() => setCurrentTestimonial((prev) => (prev - 1 + testimonials.length) % testimonials.length)} className="absolute left-0 top-1/2 -translate-y-1/2 -translate-x-4 bg-white hover:bg-gray-50 text-gray-700 p-3 rounded-full shadow-lg border border-gray-200 transition-all hover:scale-110" aria-label="Previous testimonial">
              <svg className="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M15 19l-7-7 7-7" />
              </svg>
            </button>
            <button onClick={() => setCurrentTestimonial((prev) => (prev + 1) % testimonials.length)} className="absolute right-0 top-1/2 -translate-y-1/2 translate-x-4 bg-white hover:bg-gray-50 text-gray-700 p-3 rounded-full shadow-lg border border-gray-200 transition-all hover:scale-110" aria-label="Next testimonial">
              <svg className="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        ...
        <div className="max-w-7xl mx-auto px-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
            {pricingPlans.map((plan, idx) => (
              <div key={idx} className={`rounded-xl border-2 transition-all duration-300 ${plan.highlighted ? "border-green-600 bg-green-50 shadow-xl scale-105" : "border-gray-200 bg-white hover:border-gray-300"}`}>
                <div className="p-8">
                  {plan.highlighted && (
                    <div className="bg-green-600 text-white text-sm font-semibold px-3 py-1 rounded-full inline-block mb-4">Most Popular</div>
                  )}
      ...
      <section className="py-20 bg-gradient-to-r from-green-50 to-gray-50 border-t border-gray-200">
        <div className="max-w-4xl mx-auto px-6 text-center">
          <h2 className="text-4xl font-bold text-gray-900 mb-6">Ready to Secure Your Digital Life?</h2>
          <p className="text-xl text-gray-600 mb-8">Join thousands of users who trust SafeKeyz to protect their passwords</p>
          {user ? (
            <Link to="/vault" className="inline-flex items-center justify-center gap-2 bg-green-600 hover:bg-green-700 text-white px-8 py-4 rounded-lg font-semibold transition-all duration-200 shadow-lg hover:shadow-xl">
              Go to Your Vault
              <ArrowRight size={20} />
