...
const Sidebar = () => {
  ...
  const navigationItems = [
    { icon: LayoutGrid, label: "All Items", path: "/vault-items", badge: null },
    { icon: Briefcase, label: "Vault", path: "/vault", badge: null },
    { icon: FileText, label: "Documents", path: "/documents", badge: null },
    { icon: Shield, label: "Password Health", path: "/health", badge: null },
  ];

  const categoriesItems = [
    { icon: Key, label: "Logins", path: "/vault-items?category=loginCredentials" },
    { icon: Shield, label: "Identities", path: "/vault-items?category=idCards" },
  ];
  ...
  const NavItem = ({ icon: Icon, label, path, badge, onClick }) => (
    ...
    >
      ...
      <svg className={`w-4 h-4 transition-transform ${isActive(path) ? "text-green-600" : "text-gray-400 group-hover:text-gray-600"}`} fill="none" stroke="currentColor" viewBox="0 0 24 24">
  ...
  const SectionTitle = ({ children }) => (
    <h3 className="text-xs font-semibold text-gray-600 uppercase tracking-wider px-4 mb-2 mt-6">{children}</h3>
  );
  ...
  return (
    <div className="w-64 bg-gray-50 border-r border-gray-200 flex flex-col h-screen overflow-hidden">
      ...
      <div className="p-4">
        <button onClick={() => navigate("/vault")} className="w-full bg-green-600 hover:bg-green-700 text-white font-semibold py-2.5 px-4 rounded-lg transition-all duration-200 shadow-md hover:shadow-lg">
          Get Started
        </button>
      </div>
      ...
      <div className="flex-1 overflow-y-auto px-2">
        ...
        <NavItem icon={Upload} label="Imported" path="/imported" />
        ...
        <NavItem icon={Star} label="Starter Kit" path="/tags/starter" />
      ...
      <div className="p-4 border-t border-gray-200">
        <button onClick={() => navigate("/settings")} className="w-full flex items-center gap-3 px-4 py-3 rounded-lg text-gray-700 hover:bg-gray-100 hover:text-gray-900 transition-all duration-200">
          <Settings size={20} />
